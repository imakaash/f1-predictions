...
We produce two artifacts:
    ...
    data/training.parquet  - one row per (driver, historical Monaco GP)
    ...
    data/predict_input.parquet - one row per driver entered for the upcoming
                                 ...
                                 full available history (past Monaco GPs +
                                 season form so far).
...
def _track_history_before(
    monaco_history: pd.DataFrame,
    driver: str,
    team: str,
    year: int,
) -> dict:
    """How has this driver/team done at Monaco in PREVIOUS years?"""
    prior = monaco_history[monaco_history["year"] < year]
...
def build_training() -> pd.DataFrame:
    monaco = pd.read_parquet(DATA_DIR / "monaco_history.parquet")
    monaco = monaco.sort_values("event_date").reset_index(drop=True)

    rows = []
    for _, race_row in monaco.iterrows():
        cutoff = race_row["event_date"]
        feats = _form_features_before(monaco, race_row["driver"], race_row["team"], cutoff)
        feats.update(
            _track_history_before(monaco, race_row["driver"], race_row["team"], race_row["year"])
        )
        feats.update(
            {
...
def build_predict_input() -> pd.DataFrame:
    ...
    for _, drv_row in entry.iterrows():
        ...
        feats.update(_track_history_before(monaco, driver, team, TARGET_YEAR))
        ...
        feats.update(
            {
                ...
                # Monaco FP/quali not yet run — will be NaN until fetch_data
