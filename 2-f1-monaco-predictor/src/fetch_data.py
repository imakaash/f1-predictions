...
Two datasets are produced:
    data/monaco_history.parquet  - past Monaco GPs (track-specific signal)
...
# ---------------------------------------------------------------------------
def fetch_monaco_history() -> pd.DataFrame:
    log.info("Fetching historical Monaco GP results for years %s", TRAINING_YEARS)
    frames = []
    for year in TRAINING_YEARS:
        ...
        frames.append(df)
    if not frames:
        raise RuntimeError("No historical Monaco GP data fetched.")
    out = pd.concat(frames, ignore_index=True)
    path = DATA_DIR / "monaco_history.parquet"
    out.to_parquet(path)
    out.to_csv(DATA_DIR / "monaco_history.csv", index=False)
